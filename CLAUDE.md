...
### Commerce / print
0. **Certificat d'authenticité numérique — CONSTRUIT le 18 sept. 2026, pas encore validé en conditions réelles.** Upstash Redis ajouté au projet (Vercel Marketplace, plan Free, vars `KV_REST_API_URL`/`KV_REST_API_TOKEN`). Le webhook Stripe génère désormais un code `VFC-XXXXXXXX` après chaque commande Creativehub réussie et l'écrit dans Redis ; page publique `/certificat/[code]` (`api/certificat.js`, rendue serveur, design clair imprimable, bilingue via `?lang=`) ; lien affiché sur la page de succès de la boutique dès que prêt (`api/certificat-lookup.js`, polling ~20s). Testé avec de fausses données injectées en KV (pas encore avec un vrai achat). Email Resend : pas fait, pas bloquant (le lien s'affiche déjà sur la page de succès). **Reste à faire** : 1) valider avec un vrai petit achat que le webhook génère bien un certificat correct (nom acheteur, numéro d'édition) ; 2) mettre à jour les textes du site (boutique, mentions légales) pour refléter le certificat — spec dans **`CERTIFICAT-NUMERIQUE.md`**, section "Textes site à mettre à jour" — volontairement pas encore fait pour ne rien promettre avant validation réelle.
